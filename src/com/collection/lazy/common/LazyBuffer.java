 ...
 * @author kkishore
 * 
 * Variant of SpinedBuffer, which is available from JDK1.8. 
 * Changes made with respect to LazyCollection
 * 
 * @param <E>
 */
class LazyBuffer<E> extends AbstractLazyBuffer implements Consumer<E>,
		Iterable<E> {
	...
	LazyBuffer(int initialCapacity) {
	...
	public E[] asArray(IntFunction<E[]> arrayFactory) {
		long size = count();
		if (size >= LazyConstants.MAX_ARRAY_SIZE)
			throw new IllegalArgumentException(LazyConstants.BAD_SIZE);
	...
	public Spliterator<E> spliterator() {
		class Splitr implements Spliterator<E> {
			...
			public long estimateSize() {
				return (splSpineIndex == lastSpineIndex) ? (long) lastSpineElementFence
						- splElementIndex
	...
	abstract static class OfPrimitive<E, T_ARR, T_CONS> extends
			AbstractLazyBuffer implements Iterable<E> {
		...
		protected long capacity() {
			return (spineIndex == 0) ? arrayLength(curChunk)
					: priorElementCount[spineIndex]
							+ arrayLength(spine[spineIndex]);
		}
		...
		public void copyInto(T_ARR array, int offset) {
			...
			if (spineIndex == 0)
				...
				// full chunks
				for (int i = 0; i < spineIndex; i++) {
					System.arraycopy(spine[i], 0, array, offset,
							arrayLength(spine[i]));
		...
		abstract class BaseSpliterator<T_SPLITR extends Spliterator.OfPrimitive<E, T_CONS, T_SPLITR>>
			...
			public T_SPLITR trySplit() {
				if (splSpineIndex < lastSpineIndex) {
					// split just before last chunk (if it is full this means
					// 50:50 split)
					T_SPLITR ret = newSpliterator(splSpineIndex,
							lastSpineIndex - 1, splElementIndex,
	...
	static class OfInt extends
			LazyBuffer.OfPrimitive<Integer, int[], IntConsumer> implements
			IntConsumer {
		OfInt() {
		}
		...
		public String toString() {
			...
			if (array.length < 200) {
				...
				return String.format("%s[length=%d, chunks=%d]%s...",
						getClass().getSimpleName(), array.length, spineIndex,
	...
	static class OfLong extends
			LazyBuffer.OfPrimitive<Long, long[], LongConsumer> implements
			LongConsumer {
		OfLong() {
		}
	...
	static class OfDouble extends
			LazyBuffer.OfPrimitive<Double, double[], DoubleConsumer>
